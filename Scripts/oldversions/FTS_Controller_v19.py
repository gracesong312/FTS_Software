...
######################################################################################################################
    ...
    def write_fits(self,fname):
        ...
        prihdr['NOTES'] = (str(self.note), 'Notes on the setup.')
...
######################################################################################################################   
    ...
    def inputs_check(self):
        ...
        if np.isnan(self.zopd):
            ...
            try:
                ...
                if self.reverse_btn.isChecked()==True:
                    self.throw*=-1.0*10**(-2*self.throw_index+2)
                else:
        ...
        try:
            ...
            if self.reverse_btn.isChecked()==True: 
                self.step_size*=-1.0*10**(-2*self.throw_index+2)
            else:
    ...
    def on_submit(self):
        ...
        if self.inputs_confirmed:
            ...
            self.run_scan(100*self.step_size,.5*self.throw,self.num_sample,self.speed,self.samp_rate,
