...
'''
Things to be included still:
    Catches to make sure each component is attached correctly
    Controls for the chopper (including chopping speed)
    Scrolling plots
    Proper saving format
    Finding zero OPD
    Performing the FFT (including folding the signal)
'''
...
class CustomFigCanvas(FigureCanvas, TimedAnimation):
    ...
    def _draw_frame(self, framedata):
        ...
        self.line1_tail.set_data(np.append(self.n[-10:-1], self.n[-1 - margin]), np.append(self.y[-10:-1 - margin], self.y[-1 - margin]))
...
######################################################################################################################
    ...
    def save(self):
        ...
        if fname=='':
            num_files = glob.glob('C:/Users/Paul/Desktop/FTS_Software/Data/default_fname_*.fits')
...
######################################################################################################################   
    ...
    def on_submit(self):
        ...
        self.DAQ.configure()
        plotLoop = threading.Thread(name = 'plotLoop', target = self.dataSendLoop, args = (self.addPlotData,))
        plotLoop.start()
        DAQLoop = threading.Thread(name = 'DAQLoop', target = self.read_DAQ, args = ())
        DAQLoop.start()
