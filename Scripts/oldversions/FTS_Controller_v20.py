...
class ScanThread(QThread):
    ...
    def run(self):
        ...
        while not self.exiting and self.num_steps>0:
            ...
            response = self.motor.recv(1024)
            self.motor.send('PFBK(X)\n')
            self.msleep(int(10*self.li_time)) #Sleep for 10x the LI time constant [ms]
            response = self.motor.recv(1024).strip()[1:]
            position = float(re.sub('[#$!%]','', response))
...
######################################################################################################################   
    ...
    def on_submit(self):
        ...
        if self.inputs_confirmed:
            ...
            self.run_scan(.5*self.throw,.5*self.throw,self.num_sample,self.speed,self.samp_rate,self.li_time,
