...
class CustomDAQThread(threading.Thread):
    
    def __init__(self,read_DAQ):
        threading.Thread.__init__(self)
        self.paused = False
        self.pause_cond = threading.Condition(threading.Lock())
        self.read_DAQ = read_DAQ
...
######################################################################################################################
    ...
    def find_zopd(self):
        ...
        #Check this try/catch statement for stability later.  As long as the
        #dataloop is paused before and after each scan/ZOPD it should be OK
        try:
            self.dataLoop
        except AttributeError:
            self.dataLoop = CustomDAQThread(self.read_DAQ)
        ...
        #This homes the axis each time finding ZOPD.  The flag is necessary to
        #make sure the send_home() command works properly before continue
        #through the rest of the ZOPD function
        ...
        #Slew to one end of the axis - recording position along the way
        time.sleep(15)
        self.statusBar().showMessage('Finding zero optical path difference. Negative')
        time.sleep(15)
        
        self.motorLoop.pause()
        command = 'MOVEABS X%.3f XF%.3f\n'%(-80,10)
        self.motor.send(command)
        response = self.motor.recv(1024)
        self.motorLoop.resume()
        
        time.sleep(15)
        self.statusBar().showMessage('Finding zero optical path difference. Positive')
        time.sleep(15)
        ...
        '''
        if response=='%\n':
            while self.velocity>1e-5:
                pass
        
        
        command = 'BLOCKMOTION X ON\n'
        response = self.motor.recv(1024)
        print response
        self.motor.send(command)
        ...
        response = self.motor.recv(1024)
        self.motorLoop.resume()
        while self.velocity>1e-5:pass
        '''

        '''
        self.zopd = self.position_data[np.where(self.DAQ_data==np.min(self.DAQ_data))]
...
######################################################################################################################   
    ...
    def connect_motor(self):
        ...
        try:
            ...
            self.motorLoop = CustomMotorThread()
    ...
    def read_motor(self):
        ...
        self.motor.send('VFBK(X)\n')
        response = self.motor.recv(1024).strip()[1:]
        response = re.sub('[#$!%]','', response)
        self.velocity = float(response)
...
class CustomMotorThread(Window,threading.Thread):
    
    def __init__(self):
        super(CustomMotorThread,self).__init__()
        #threading.Thread.__init__(self)
        self.paused = False
        self.pause_cond = threading.Condition(threading.Lock())

    def read_motor(self):
        super(CustomMotorThread,self).read_motor()
