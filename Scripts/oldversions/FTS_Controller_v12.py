...
class ContinuousThread(QThread):
    ...
    def run(self):
        while not self.exiting:
            while self.paused:
                pass
            time.sleep(.1) #100ms feedback time
            self.motor.send('PFBK(X)\n')
            response = self.motor.recv(1024).strip()[1:]
            position = float(re.sub('[#$!%]','', response))
            self.update.emit(position)
 

class ScanThred(QThread):
    
    update_position = pyqtSignal(float)
    update_plot = pyqtSignal(tuple)
    
    def __init__(self,motor,interferogram,dx,num_steps,num_samples):
    ...
    def run(self):
        ...
        while not self.exiting and num_steps>0:
            while self.paused:
                pass
            ...
            self.update_plot.emit((position,interferogram(position)))
            num_steps-=1
...
######################################################################################################################   
    ...
    def connect_motor(self):
        ...
        try:
            ...
            self.motorLoop.update.connect(self.update_position)
            self.motorLoop.start()
