...
class ScanThread(QThread):
    ...
    def __init__(self,motorLoop,motor,position_data,DAQ_data,interferogram,step_size,throw,speed,num_samples,plot_scan):
...
######################################################################################################################
    ...
    def run_scan(self,step_size,throw,num_samples,speed,
        ...
        self.scanthread = ScanThread(self.motorLoop,self.motor,self.position_data,self.DAQ_data,self.interferogram,step_size,throw,speed,num_samples,plot_scan)
    ...
    def find_zopd(self):
        ...
        def zopd_scan():
            ...
            self.run_scan(-.1,10,1,1,thread_text,finished_text,"finished()",determine_zopd,True,False) #Slew 10mm to the left in steps of .1mm at 1mm/s
        ...
        self.run_scan(.1,5,1,1,status_text,status_text,"finished()",zopd_scan,False,False) #Slew 5mm to the right in steps of .1mm at 1mm/s
    ...
    def on_submit(self):
        ...
        #self.systems_check()        
            ...
            def full_scan():
                ...
                self.run_scan(-1.*self.step_size,np.ceil(abs(self.throw/self.step_size)),self.num_sample,self.speed,
                thread_text,finished_text,"finished()",self.do_nothing,True,False)
                    
            #Now run the scan
            status_text = 'Performing scan'
            self.run_scan(self.step_size,.5*np.ceil(abs(self.throw/self.step_size)),self.num_sample,self.speed,
