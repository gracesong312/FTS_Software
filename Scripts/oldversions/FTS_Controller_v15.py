...
class ScanThread(QThread):
    ...
    def __init__(self,motorLoop,motor,position_data,DAQ_data,interferogram,dx,num_steps,num_samples,plot_scan):
        ...
        self.interferogram = interferogram #This will become the DAQ
        self.dx = dx
    ...
    def run(self):
        ...
        while not self.exiting and self.num_steps>0:
            ...
            self.send_position.emit(position)
            if self.plot_scan:
                self.send_data.emit((position,self.interferogram(position)))
            self.position_data.append(position)
            self.DAQ_data.append(self.interferogram(position))
            self.num_steps-=1
            self.msleep(10)
        xf,fft_data = self.takeFFT()
        #print fft_data
        self.xf = xf
    ...
    def takeFFT(self):
        fft_data = abs(fftpack.fft(self.DAQ_data))
        N = len(self.DAQ_data)
        xf = self.position_data/(2*np.max(self.position_data))*N
        return xf,fft_data
        #ax1 = self.f1.add_subplot(1,1,1)
        #ax1.plot(xf[:N//2],abs(fft_data[:N//2]))
        #self.f1.canvas.draw()
...
######################################################################################################################
    ...
    def fft_plot(self,x,y):
        fft_data = fftpack.fft(y)
        N = len(x)
        xf = x/(2*np.max(x))*N
        ax1 = self.f1.add_subplot(1,1,1)
        ax1.plot(xf[:N//2],abs(fft_data[:N//2]))
    ...
    def run_scan(self,dx,num_steps,num_samples,
        ...
        self.scanthread = ScanThread(self.motorLoop,self.motor,self.position_data,self.DAQ_data,self.interferogram,dx,num_steps,num_samples,plot_scan)
    ...
    def find_zopd(self):
        ...
        def zopd_scan():
            ...
            self.run_scan(-.1,20,1,thread_text,finished_text,"finished()",determine_zopd,True,False)
        ...
        self.run_scan(.1,10,1,status_text,status_text,"finished()",zopd_scan,False,False)
...
######################################################################################################################        
    ...
    def create_main_frame(self):
        ...
        step_label = self.create_qlabel('# Steps:',120,20,'left',self.general_font)
        self.step_line = self.create_qline(215,20,font=self.general_font)
...
######################################################################################################################
    ...
    def write_fits(self,fname):
        ...
        prihdr['NSTEPS'] = (int(self.num_sample),'Number of steps in scan')
...
######################################################################################################################   
    ...
    def inputs_check(self):
        ...
        #Samples check
        try:
            self.num_step = int(self.step_line.text())
            checks.append('# Steps: CLEAR\n\n')
        except:
            checks.append('# Steps: Please enter a valid number.\n\n')
        ...
        try:
            self.temperature = float(self.source_temp_line.text())
        except:
    ...
    def confirm_inputs(self):
        ...
        if (self.source=='N/A') and (self.temperature=='N/A'):
            msg_txt += 'Source: N/A\n\n'
        elif (self.source!='N/A') and (self.temperature=='N/A'):
            msg_txt += 'Source: '+self.source+' @ ~~K\n\n'
        elif (self.source=='N/A') and (self.temperature!='N/A'):
            msg_txt += 'Source: N/A @ '+'%i'%self.temperature+' K\n\n'
        else:
            msg_txt += 'Source: '+self.source+' @ '+'%i'%self.temperature+' K\n\n'    
        if self.press=='N/A':
    ...
    def takeFFT(self):
        fft_data = abs(fftpack.fftshift(fftpack.fft(self.scanthread.DAQ_data)))
        N = len(self.scanthread.DAQ_data)
        xf = self.scanthread.position_data/(2*np.max(self.scanthread.position_data))*N
        return xf,fft_data
        #ax1 = self.f1.add_subplot(1,1,1)
        #ax1.plot(xf[:N//2],abs(fft_data[:N//2]))
        #self.f1.canvas.draw()
    ...
    def on_submit(self):
        ...
        if self.inputs_confirmed:
            ...
            def full_scan():
                ...
                self.run_scan(-.1,200,10,thread_text,finished_text,"finished()",None,True,True)
                    
            #Now run the scan
            status_text = 'Performing scan'
            self.run_scan(.1,200,10,status_text,status_text,"finished()",full_scan,True,True)
            time.sleep(25)
            xf,fft_data = self.takeFFT()
            self.fft_plot(xf,fft_data)
