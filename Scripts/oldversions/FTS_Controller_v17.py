...
class ScanThread(QThread):
    ...
    def __init__(self,motorLoop,motor,position_data,DAQ_data,DAQ,step_size,throw,speed,plot_scan):
...
######################################################################################################################
    ...
    def run_scan(self,step_size,throw,num_samples,speed,sample_rate,
        ...
        self.scanthread = ScanThread(self.motorLoop,self.motor,self.position_data,self.DAQ_data,self.DAQ,step_size,throw,speed,plot_scan)
    ...
    def find_zopd(self):
        ...
        def zopd_scan():
            ...
            self.run_scan(-.1,10,2,1,40,thread_text,finished_text,"finished()",determine_zopd,True,False) #Slew 10mm to the left in steps of .1mm at 1mm/s
            
        #Create the slot to send the finsihed signal to for the ZODP scan
        def determine_zopd():
            self.DAQ_data = np.average(self.DAQ_data,axis=1)
            self.zopd = self.position_data[np.where(self.DAQ_data==np.max(self.DAQ_data))[0][0]]
        ...
        self.run_scan(.1,5,2,1,40,status_text,status_text,"finished()",zopd_scan,False,False) #Slew 5mm to the right in steps of .1mm at 1mm/s
...
######################################################################################################################        
    ...
    def create_main_frame(self):
        ...
        step_items = ['cm','mm']
        self.step_combo = self.create_qcombobox(step_items,65,20,self.general_font,1)
        step_box.addWidget(step_label)
        step_box.addWidget(self.step_line)
        step_box.addWidget(self.step_combo)
        step_box.addStretch()
...
######################################################################################################################
    ...
    def write_fits(self,fname):
        ...
        prihdr['SOURCE TEMP'] = (self.temp,'Temperature of the source used')
        ...
        prihdr['NSAMPLES'] = (int(self.num_sample),'Number of samples at each step')
        prihdr['NSTEPS'] = (int(np.ceil(abs(self.throw/self.step_size))),'Number of steps in scan')
        prihdr['SAMPLE RATE'] = (self.samp_rate,'Sampling rate in '+self.samp_rate_combo.currentText())
        prihdr['TIME CONSTANT'] = (self.li_time,'Time constant of the lock-in amplifier')
        prihdr['CHOP RATE'] = (self.chop_rate, 'Chopping frequency in Hz')
        prihdr['DATE'] = (datetime.datetime.now().isoformat(), 'Date and time when the scan was completed')
        ...
        ydhdu = pyfits.TableHDU.from_columns(
                [pyfits.Column(name='DAQ_data',format='E15.7',array=self.DAQ_data)])
        ...
        hdulist = pyfits.HDUList([prihdu,xdhdu,ydhdu])
...
######################################################################################################################   
    ...
    def systems_check(self):
        ...
        '''
        try:
            stepper = serial.Serial('COM5',9600,timeout = 1)
            stepper.close()
            stepper_check = 'Stepper check: CLEAR.\n\n'
        except:
            stepper_check = 'Problem with connecting the stepper.\n'+\
                            'Please make sure that the stepper motor is connected on COM5.\n\n'
    ...
    def inputs_check(self):
        ...
        try:
            ...
            self.speed*=10**(-2*self.speed_index+2)
            if self.speed>350:
        ...
        if np.isnan(self.zopd):
            ...
            try:
                ...
                self.throw*=10**(-2*self.throw_index+2)
                if self.throw+abs(self.zopd)>80:
        ...
        try:
            ...
            self.step_index = self.throw_combo.currentIndex()
            self.step_size*=10**(-2*self.throw_index+2)
            if self.step_size>self.throw:
        ...
        try:
            ...
            self.samp_rate*=10**(-2*self.samp_rate_index+2)
            if self.samp_rate>40:
                checks.append('Sampling Rate: Must be <=40Hz.\n\n')
            else:
    ...
    def confirm_inputs(self):
        ...
        msg_txt += 'Step Size: %4.3f'%self.step_size+'\n\n'
    ...
    def on_submit(self):
        ...
        if self.inputs_confirmed:
            ...
            def full_scan():
                ...
                self.samp_rate,thread_text,finished_text,"finished()",self.do_nothing,True,False) 
                    
            #Now run the scan
            status_text = 'Performing scan'
            self.run_scan(self.step_size,.5*self.throw,self.num_sample,self.speed,self.samp_rate,
