...
class CustomThread(QThread):
    ...
    def stop(self): self.exiting = True; self.wait()
        
    def pause(self): self.paused = True; self.wait()
    
    def resume(self): self.paused = False; self.wait()
...
######################################################################################################################
    ...
    def find_zopd(self):
        ...
        num_steps = int(3/.1)
        counter=0
        while not self.paused and counter<num_steps:
            while self.paused:
                pass
    ...
    def goto_zopd(self):
        ...
        if np.isnan(self.zopd):
            ...
            try:      
                ...
                for i in range(int(abs(self.zopd-self.position)/.1)):
                    if self.zopd<self.position:
                        self.motor.send('MOVEINC X-.1\n')
                        response = self.motor.recv(1024)
                    else:
                        self.motor.send('MOVEINC X .1\n')
                ...
                #Now move the fine scale motion to ZOPD    
                self.motor.send('MOVEABS X%.5f\n'%self.zopd)
                response = self.motor.recv(1024)
                self.read_motor()
                if response=='%\n':
