...
class ScanThread(QThread):
    ...
    def __init__(self,motorLoop,motor,interferogram,dx,num_steps,num_samples,plot_scan):
...
######################################################################################################################
    ...
    def read_DAQ(self):
        mySrc = Communicate()
        mySrc.data_signal.connect(self.update_plot)
        time.sleep(.01)
    ...
    def run_scan(self,dx,num_steps,num_samples,
        ...
        self.thread = ScanThread(self.motorLoop,self.motor,self.interferogram,dx,num_steps,num_samples,plot_scan)
        self.thread.send_position.connect(self.update_position)
        self.thread.send_data.connect(self.update_plot)
        if signal and slot:
            self.connect(self.thread, SIGNAL(signal), slot)        
        if plot_fft:
            self.connect(self.thread, SIGNAL("finished()"), self.do_nothing)
        self.thread.start()
    ...
    def find_zopd(self):
        ...
        def zopd_scan():
            ...
            finished_text = 'Found ZOPD'
            self.run_scan(-.1,200,1,thread_text,finished_text,None,None,True,False)
        
        #Now run the scan
        status_text = 'Finding zero optical path difference.'
        self.run_scan(.1,100,1,status_text,status_text,"finished()",zopd_scan,False,False)
