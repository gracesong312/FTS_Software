...
class CustomDAQThread(threading.Thread):
    ...
    def __init__(self,f2,DAQ,DAQ_data,position,position_data,interferogram):
        threading.Thread.__init__(self)
        self.paused = False
        self.pause_cond = threading.Condition(threading.Lock())
        self.f2 = f2
        ...
        #self.counter = 0 #Counter only to generate synthetic positions
...
######################################################################################################################
    ...
    def find_zopd(self):
        ...
        #Make sure a dataloop is running
        try:
            self.dataLoop
            self.dataLoop.pause()
            ...
            self.motor_data = []
            self.position_data = []
            self.dataLoop = CustomDAQThread(self.f2,self.DAQ,self.DAQ_data,self.position,self.position_data,self.interferogram)
        ...
        command = 'MOVEINC X%.3f XF%.3f\n'%(80,10) #Slew to one end of the axis
        ...
        velocity = float(response)
        ''' 
        while self.velocity>1e-5:pass
        self.DAQ_data = []
        self.position_data = []    
        self.motorLoop.pause()               
        command = 'MOVEINC X%.3f XF%.3f\n'%(-160,10) #Slew the entire axis
        self.motor.send(command)
        response = self.motor.recv(1024)
        if response=='%\n':
            self.dataLoop.resume()
...
######################################################################################################################   
    ...
    def connect_motor(self):
        ...
        self.motor = socket.socket(socket.AF_INET, socket.SOCK_STREAM) #Init socket (TCP/IP)
        self.motor.settimeout(3600) #Connection willl automatically close if dormant for 1 houe
        self.motor.connect((self.HOST,self.PORT)) #Bind HOST,PORT to socket    
