...
atm_data = []
si_data = []
pos = np.linspace(-26.5,13.495,800)
dx=.05
...
#We drop the first scan because of the unexpected slope in the data
#Not entirely clear what could have cause the parabolic shape
si_data = si_data[1:,:]

#We remove one of the atmosphere scans because of a particularly large
#drop in the signal
atm_data = np.delete(atm_data,2,axis=0)
...
si_sim = np.loadtxt('C:/Users/Paul/Documents/FTS/si_wafer_1000um_45-2000GHz.csv',skiprows=1,delimiter=',')

Fs_prime = atm_Fs*(1-np.diff(atm_Fs)[0]/(2*np.max(atm_Fs)))
from matplotlib import pyplot as plt
plt.plot(Fs_prime*1000,si_psd/atm_psd)
...
#plt.legend(['Hi-res Average','Lo-res Average'],prop={'size':20},frameon=False)